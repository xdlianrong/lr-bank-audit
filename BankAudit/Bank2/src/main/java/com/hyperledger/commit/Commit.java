...
//        }
         ...
         * 读取银行1的资产
         */
        BigInteger assets = ReadBigInteger.readBigInteger("bank2AssetsRecord.txt");
...
//        comarr[1] = (arr[2].modPow(BigInteger.valueOf(0), arr[0])).multiply((arr[3]).modPow(r2, arr[0])).mod(arr[0]);
             ...
             */
            BigInteger transactionTime = ReadBigInteger.readBigInteger("bank2TransactionTime.txt");
            FileWriter fileTransactionTime = new FileWriter("bank2TransactionTime.txt");
            ...
            FileWriter fileDoubleComSum = new FileWriter("bank2DoubleComSum.txt");
            ...
            FileWriter fileTripleComSum = new FileWriter("bank2TripleComSum.txt");
            ...
            FileWriter fileQuadraComSum = new FileWriter("bank2QuadraComSum.txt");
...
//        comarr[44] = new BigInteger(lxStr);
            //lx向量不好存储，先放进文件，在读取
            FileWriter fileLx = new FileWriter("bank2FileLx.txt");
            fileLx.write(lxStr + "\n");
            fileLx.close();
            //rx
...
//        comarr[45] = new BigInteger(rxStr);
            //t
            //rx向量不好存储，先放进文件，在读取
            FileWriter fileRx = new FileWriter("bank2FileRx.txt");
            fileRx.write(rxStr + "\n");
            fileRx.close();
            ...
            FileWriter file2 = new FileWriter("bank2AssetsRecord.txt");
...
//        FileWriter file = new FileWriter("bank1.txt",true);
            FileWriter file1 = new FileWriter("bank2random.txt");
...
//        }
        ...
        if (bankStr == 1){
            SendMessage.sendMesssage("4bank2", m.toString(),"127.0.0.1", 8881);
        }else if (bankStr == 3){
            SendMessage.sendMesssage("4bank2", m.toString(),"127.0.0.1", 8883);
        }else if (bankStr == 4){
            SendMessage.sendMesssage("4bank2", m.toString(),"127.0.0.1", 8884);
        }
...
//        System.out.println("成功写入文件！");
    ...
    public static void commitAll(String str, BigInteger value, BigInteger[] arr) throws IOException {
        //首先判断要给那个银行转账，本程序的以bank2连接的节点，所以是以bank2作为支出银行的。
        Bank bank2 = new Bank();
        BigInteger[] bank2arr = commitSelfBank(value, arr);
...
//            System.out.println(bank4.getLx());
             ...
             * 该文件记录银行2的资产转给了哪个银行
             */
            FileWriter file = new FileWriter("bank2AssetsRecord1.txt",true);
            file.write("bank2 -> "+"bank1:" + value + "\n");
            file.close();

...
//        }
         ...
         * 读取银行1的资产
         */
        BigInteger assets = ReadBigInteger.readBigInteger("bank3AssetsRecord.txt");
...
//        comarr[1] = (arr[2].modPow(BigInteger.valueOf(0), arr[0])).multiply((arr[3]).modPow(r2, arr[0])).mod(arr[0]);
             ...
             */
            BigInteger transactionTime = ReadBigInteger.readBigInteger("bank3TransactionTime.txt");
            FileWriter fileTransactionTime = new FileWriter("bank3TransactionTime.txt");
             ...
             * 交易额的平方写进文件
             */
            BigInteger doubleComSum = ReadBigInteger.readBigInteger("bank3DoubleComSum.txt");
            FileWriter fileDoubleComSum = new FileWriter("bank3DoubleComSum.txt");
             ...
             * 交易金额的立方写进文件
             */
            BigInteger tripleComSum = ReadBigInteger.readBigInteger("bank3TripleComSum.txt");
            FileWriter fileTripleComSum = new FileWriter("bank3TripleComSum.txt");
             ...
             * 交易金额的四次方写进文件
             */
            BigInteger quadraComSum = ReadBigInteger.readBigInteger("bank3QuadraComSum.txt");
            FileWriter fileQuadraComSum = new FileWriter("bank3QuadraComSum.txt");
...
//        comarr[44] = new BigInteger(lxStr);
            //lx向量不好存储，先放进文件，在读取
            FileWriter fileLx = new FileWriter("bank3FileLx.txt");
            fileLx.write(lxStr + "\n");
            fileLx.close();
            //rx
...
//        comarr[45] = new BigInteger(rxStr);
            //t
            //rx向量不好存储，先放进文件，在读取
            FileWriter fileRx = new FileWriter("bank3FileRx.txt");
            fileRx.write(rxStr + "\n");
            fileRx.close();
            ...
            FileWriter file2 = new FileWriter("bank3AssetsRecord.txt");
...
//        FileWriter file = new FileWriter("bank1.txt",true);
            FileWriter file1 = new FileWriter("bank3random.txt");
...
//        }
        ...
        if (bankStr == 1){
            SendMessage.sendMesssage("4bank3", m.toString(),"127.0.0.1", 8881);
        }else if (bankStr == 2){
            SendMessage.sendMesssage("4bank3", m.toString(),"127.0.0.1", 8882);
        }else if (bankStr == 4){
            SendMessage.sendMesssage("4bank3", m.toString(),"127.0.0.1", 8884);
        }
...
//        System.out.println("成功写入文件！");
    ...
    public static void commitAll(String str, BigInteger value, BigInteger[] arr) throws IOException {
        //首先判断要给那个银行转账，本程序的以bank3连接的节点，所以是以bank3作为支出银行的。
        Bank bank3 = new Bank();
        BigInteger[] bank3arr = commitSelfBank(value, arr);
...
//            System.out.println(bank4.getLx());
             ...
             * 该文件记录银行3的资产转给了哪个银行
             */
            FileWriter file = new FileWriter("bank3AssetsRecord1.txt",true);
            file.write("bank3 -> "+"bank1:" + value + "\n");
            file.close();

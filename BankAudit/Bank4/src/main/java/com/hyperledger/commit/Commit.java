...
//        }
        BigInteger r1temp = GetRandom.getRandom(arr);
        r1 = BigInteger.valueOf(0).subtract(r1temp).mod(arr[1]);
...
//        }
         ...
         * 读取银行1的资产
         */
        BigInteger assets = ReadBigInteger.readBigInteger("bank4AssetsRecord.txt");
...
//        comarr[1] = (arr[2].modPow(BigInteger.valueOf(0), arr[0])).multiply((arr[3]).modPow(r2, arr[0])).mod(arr[0]);
             ...
             */
            BigInteger transactionTime = ReadBigInteger.readBigInteger("bank4TransactionTime.txt");
            FileWriter fileTransactionTime = new FileWriter("bank4TransactionTime.txt");
             ...
             * 交易额的平方写进文件
             */
            BigInteger doubleComSum = ReadBigInteger.readBigInteger("bank4DoubleComSum.txt");
            FileWriter fileDoubleComSum = new FileWriter("bank4DoubleComSum.txt");
             ...
             * 交易金额的立方写进文件
             */
            BigInteger tripleComSum = ReadBigInteger.readBigInteger("bank4TripleComSum.txt");
            FileWriter fileTripleComSum = new FileWriter("bank4TripleComSum.txt");
             ...
             * 交易金额的四次方写进文件
             */
            BigInteger quadraComSum = ReadBigInteger.readBigInteger("bank4QuadraComSum.txt");
            FileWriter fileQuadraComSum = new FileWriter("bank4QuadraComSum.txt");
...
//        comarr[44] = new BigInteger(lxStr);
            //lx向量不好存储，先放进文件，在读取
            FileWriter fileLx = new FileWriter("bank4FileLx.txt");
            fileLx.write(lxStr + "\n");
            fileLx.close();
            //rx
...
//        comarr[45] = new BigInteger(rxStr);
            //t
            //rx向量不好存储，先放进文件，在读取
            FileWriter fileRx = new FileWriter("bank4FileRx.txt");
            fileRx.write(rxStr + "\n");
            fileRx.close();
            ...
            FileWriter file2 = new FileWriter("bank4AssetsRecord.txt");
...
//        FileWriter file = new FileWriter("bank1.txt",true);
            FileWriter file1 = new FileWriter("bank4random.txt");
...
//        }
        ...
        if (bankStr == 1){
            SendMessage.sendMesssage("4bank4", m.toString(),"127.0.0.1", 8881);
        }else if (bankStr == 2){
            SendMessage.sendMesssage("4bank4", m.toString(),"127.0.0.1", 8882);
        }else if (bankStr == 3){
            SendMessage.sendMesssage("4bank4", m.toString(),"127.0.0.1", 8883);
        }
...
//            FileWriter file1 = new FileWriter("bank2.txt",true);
            FileWriter file4 = new FileWriter("bank1random.txt");
//            FileWriter fileRandom = new FileWriter("bank1random1.txt",true);
...
//            fileRandom.close();
        ...
        } else if (str.equals("bank2")){
//            FileWriter file2 = new FileWriter("bank3.txt",true);
            FileWriter file5 = new FileWriter("bank2random.txt");
...
//            FileWriter file3 = new FileWriter("bank4.txt",true);
            FileWriter file6 = new FileWriter("bank3random.txt");
...
//        System.out.println("成功写入文件！");
    ...
    public static void commitAll(String str, BigInteger value, BigInteger[] arr) throws IOException {
        //首先判断要给那个银行转账，本程序的以bank4连接的节点，所以是以bank4作为支出银行的。
        Bank bank4 = new Bank();
        BigInteger[] bank4arr = commitSelfBank(value, arr);
...
//            System.out.println(bank4.getLx());
        ...
        if(str.equals("bank1")){
            //银行1的交易信息
            Bank bank1 = new Bank();
            BigInteger[] bank1arr = commitOtherBank("bank1", value, arr);
...
//            System.out.println(bank3.getLx());
             ...
             * 该文件记录银行4的资产转给了哪个银行
             */
            FileWriter file = new FileWriter("bank4AssetsRecord1.txt",true);
            file.write("bank4 -> "+"bank1:" + value + "\n");
            file.close();
            ...
            //银行1的交易信息
            Bank bank1 = new Bank();
            BigInteger[] bank1arr = commitOtherBank("bank1", BigInteger.valueOf(0), arr);
            bank1.setCom(bank1arr[0]);
            bank1.setBitCom(bank1arr[1]);
            bank1.setisFrom((bank1arr[2].intValue()));
            bank1.setDoubleCom(bank1arr[3]);
